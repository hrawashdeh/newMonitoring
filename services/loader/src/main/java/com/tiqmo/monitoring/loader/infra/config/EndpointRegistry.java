...
import com.tiqmo.monitoring.loader.domain.config.repo.ApiEndpointRepository;
...
public class EndpointRegistry {
    ...
    @Value("${spring.application.name:loader}")
    private String serviceName;

    @Value("${api.config.service-id:ldr}")
    ...
    /**
     * Discover all endpoints annotated with @ApiKey.
     */
    private List<EndpointInfo> discoverEndpoints() {
        List<EndpointInfo> endpoints = new ArrayList<>();

        // Get all beans with RestController annotation
    ...
    /**
     * Get base path from @RequestMapping on controller class.
     */
    ...
    /**
     * Get method path from mapping annotations.
     */
    ...
    /**
     * Get HTTP method from mapping annotations.
     */
    ...
    /**
     * Persist endpoints to database for long-term storage and management UI.
     * Uses upsert logic: insert new endpoints, update existing ones.
     */
    private void persistToDatabase(List<EndpointInfo> endpoints) {
        ...
        // Mark old endpoints as REMOVED (not seen in this registration)
        try {
            Instant threshold = now.minusSeconds(60); // Give 60 seconds tolerance
